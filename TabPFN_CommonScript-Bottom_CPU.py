...
if KeepSplit:
    ...
    X_train, X_test, y_train, y_test = train_test_split(
        X, y, 
        test_size = test_size,         # 20% test data
...
# --- Limit to first 10000 rows if needed ---
...
models = {
    "AdaBoost": AdaBoostClassifier(n_estimators=50, random_state=91311),
    "Bagging": BaggingClassifier(n_estimators=50, random_state=91311),
    "CatBoost": CatBoostClassifier(iterations=100, verbose=0, random_state=91311),
    "DecisionTree": DecisionTreeClassifier(random_state=91311),
    "ExtraTrees": ExtraTreesClassifier(n_estimators=100, random_state=91311),
    "GradientBoosting": GradientBoostingClassifier(n_estimators=100, random_state=91311),
    "KNN": KNeighborsClassifier(n_neighbors=5),
    "LightGBM": LGBMClassifier(n_estimators=100, random_state=91311),
    "LogisticRegression": LogisticRegression(max_iter=500, random_state=91311),
    "MLP": MLPClassifier(hidden_layer_sizes=(100,), max_iter=500, random_state=91311),
    "RandomForest": RandomForestClassifier(n_estimators=50, max_depth=3, random_state=91311),
    "SVC": SVC(kernel="rbf", probability=True, random_state=91311)
}



existing_results_file = f"{VOI}_Youden_model_metrics_{old}.csv"
print(f"Checking for {existing_results_file}")

# Check for any existing results file with the same VOI
existing_files = glob.glob(f"{VOI}_Youden_model_metrics_*.csv")
...
if run_ml_loop:
    ...
    for model_name, model in models.items():
        ...
        pattern = f"{VOI}_{model_name}_bootstrap_metrics_*.csv"
        ...
        if existing_bootstrap_files:
            ...
            now = datetime.now().strftime("%Y%m%d-%H%M-%S")
            bootstrap_metrics_file = f"{VOI}_{model_name}_bootstrap_metrics_{now}.csv"
        ...
        try:
            ...
            model_runtime_loops[model_name] = loop_runtimes            # model_runtime_loops[model_name] = loop_runtimes

            # Aggregate per-model summary if we collected any metrics
            if all_bootstrap_metrics:
                df_all = pd.DataFrame(all_bootstrap_metrics)
                df_all.to_csv(f"{VOI}_{model_name}_bootstrap_metrics_{now}.csv", index=False)
    ...
    plot_df.to_csv(f"{VOI}_ALL_PLUS_5Fold_model_metrics_{now}.csv", index=False)
    ...
    plt.savefig(f'{VOI}_BarPlot_{now}.png', dpi=300, bbox_inches='tight')
    plt.show()
    ...
    plt.ylabel('Model', fontsize=14)
    plt.tight_layout(rect=[0, 0.03, 1, 0.95])
    plt.savefig(f'{VOI}_RuntimePerLoop_BarPlot_{now}.png', dpi=300, bbox_inches='tight')
    plt.show()
